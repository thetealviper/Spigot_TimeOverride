...
public class EnableShit {
	...
	public static void handleOnEnable(JavaPlugin plugin, Listener pluginL, String spigotID){
		...
		Bukkit.getServer().getConsoleSender().sendMessage(TimeOverride.LOG_PREFIX + plugin.getDescription().getName() + " from TheTealViper powered ON!");
	}
	...
	static void updatePlugin(JavaPlugin plugin, String spigotID){
		...
		for(int i = 0;i < posted_Arr.length;i++){
			if(installed_Arr.length <= i || Integer.valueOf(installed_Arr[i]) < Integer.valueOf(posted_Arr[i])){
				Bukkit.getServer().getConsoleSender().sendMessage(TimeOverride.LOG_PREFIX + plugin.getDescription().getName() + " has an update ready [" + installed + " -> " + posted + "]!");
				break;
			}
		}
	}
	static void updateConfig(JavaPlugin plugin){
		...
		if(update){
			...
			if(file.exists())
				file.delete();
			plugin.saveDefaultConfig();
			Bukkit.getServer().getConsoleSender().sendMessage(TimeOverride.LOG_PREFIX + plugin.getDescription().getName() + " config.yml has been updated [" + oldVersion + " -> " + newVersion + "] and a backup created of old configuration!");
		}
	}
